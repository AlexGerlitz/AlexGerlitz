...
- QA automation, API testing, Python checks, SQL/data checks, smoke checks, logs, and issue notes.
- Integration work around CRM, ERP, APIs, webhooks, Telegram, 1C-facing workflows, banking-facing
  workflows, audit, retries, idempotency, and privacy boundaries.
...
- Russia-first remote search: prioritize Python backend/internal tools, QA/API verification with
  ...
  call; keep the exact minimum screen in private outreach instead of the public work sample.
- Europe-compatible search: keep English-first docs, async review paths, privacy boundaries, CI,
...
runbooks, and handoff docs as part of backend/internal-tools delivery. I do not lead with senior
DevOps, Kubernetes, Terraform, or cloud platform ownership as the primary claim.

I use AI tooling to compress discovery, implementation, debugging, docs, and review, while
architecture, state boundaries, privacy checks, tests, logs, deployment notes, runbooks, and shipped
quality stay my responsibility.

I do not attach live admin captures, real names, phone numbers, chat IDs, private routes, logs,
dumps, record identifiers, tokens, or credentials. Public evidence uses synthetic or redacted data.

## ATS Keyword Bundle

| Lane | Titles | Keywords | First review link |
| --- | --- | --- | --- |
| Python backend / internal tools | Junior Backend Developer, Junior Python Developer, Back End Developer, Python Developer, Internal Tools Developer | Python, FastAPI, PostgreSQL, SQL, REST API, OpenAPI, pytest, Docker Compose, GitHub Actions, admin workflow, Telegram bot, handoff docs | [Application Fit Pack](https://alexgerlitz.github.io/AlexGerlitz/application-fit-pack.html) -> [First Backend Role Fit](https://alexgerlitz.github.io/AlexGerlitz/first-backend-role.html) -> [Autoschool Intake/Admin work sample](https://alexgerlitz.github.io/AlexGerlitz/autoschool-intake-admin.html) |
...
| Integration / API / CRM | Integration Engineer, API Integration Engineer, CRM Integration Engineer, Workflow Integration Engineer | CRM, ERP, API integration, webhooks, adapter contracts, retries, idempotency, audit logs, privacy boundaries, operator handoff | [Autoschool Intake/Admin work sample](https://alexgerlitz.github.io/AlexGerlitz/autoschool-intake-admin.html) -> [DriveDesk AI Operator](https://alexgerlitz.github.io/AlexGerlitz/drivedesk-proof-route.html) -> [Verification Pack](https://alexgerlitz.github.io/AlexGerlitz/verification-pack.html) |
| AI workflow / RAG | AI workflow sample, RAG workflow sample, backend automation with AI workflow ownership | RAG, retrieval, structured output, approvals, citations, eval checks, transcript analysis, CRM-safe handoff, runbooks | [AI Automation Role Fit](https://alexgerlitz.github.io/AlexGerlitz/ai-automation-role-fit.html) -> [AI Backend Review Pack](https://alexgerlitz.github.io/AlexGerlitz/ai-backend-proof-pack.html) -> [DriveDesk AI Operator](https://alexgerlitz.github.io/AlexGerlitz/drivedesk-proof-route.html) |
...
```text
Hi, I am interested in this remote Python/backend role. My strongest overlap is Python, FastAPI, PostgreSQL/SQL, REST API, OpenAPI, pytest, Docker Compose/GitHub Actions handoff, and internal admin workflows. Fast review path: Application Fit Pack -> First Backend Role Fit -> Autoschool Intake/Admin work sample. I can start with one bounded workflow slice: input, validation, database state, output, verification, and handoff.
```

### QA/API Verification

```text
Hi, I am interested in this QA/API automation role. I can use Python, pytest, REST/OpenAPI checks, SQL/data state checks, logs, smoke checks, and issue notes to make one business workflow repeatably verifiable. Fast review path: Backend Role Fit -> Skill Evidence -> Verification Pack.
```

### Integration / API / CRM

```text
Hi, I am interested in this integration/API role. I can map one source and one target system, define the adapter contract, privacy boundary, retries/idempotency/audit needs, and leave a verifiable first sync or intake/admin slice. Fast review path: Autoschool Intake/Admin work sample -> DriveDesk AI Operator -> Enterprise Readiness.
```
...
```text
Thanks. Send the role, remote setup, stack, first-month ownership surface, review path, timeline, and compensation band if shareable. I will reply with fit, risks, the smallest responsible first slice, and the review path to inspect first.
```
...
Yes. I should be evaluated by a bounded first slice and public evidence, not by vague seniority labels.
Since March 2024 I have supported real business digital operations and turned that context into
public-safe work samples: Telegram request intake, backend validation, database state, admin queue,
